...
The community platform [MINTvernetzt](www.mint-vernetzt-community.de) provides a feature where the users can select the regions in which they are active. Consequently, there
needs to be a list of all states and districts (in this case for Germany), which is exactly what this package provides. It contains the Prisma schema and a script to generate the data itself.
